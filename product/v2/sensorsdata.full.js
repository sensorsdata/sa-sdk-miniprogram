...
function apiStaging() {
  var saApiList = ['resetAnonymousIdentity', 'setProfile', 'setOnceProfile', 'track', 'quick', 'incrementProfile', 'appendProfile', 'login', 'logout', 'registerApp', 'register', 'clearAllRegister', 'clearAllProps', 'clearAppRegister', 'bind', 'unbind', 'bindOpenid', 'unbindOpenid', 'bindUnionid', 'unbindUnionid'];
...
var oldOpenid = {
  init(sa) {
    ...
    var openid = {
            ...
            if (res.code && sa.para.appid && sa.para.openid_url) {
              sa._.wxrequest({
                ...
                complete: function (res2) {
                  if (sa._.isObject(res2) && sa._.isObject(res2.data) && res2.data.openid) {
      ...
      getWXStorage: function () {
        ...
        if (storageInfo && sa._.isObject(storageInfo)) {
    ...
    sa.unsetOpenid = function (val) {
      ...
      var name = sa._.getOpenidNameByAppid();
      if (Object.prototype.hasOwnProperty.call(sa.store._state.identities, name) && id === sa.store._state.identities[name]) {
        delete sa.store._state.identities[name];

        // 2022-11-29-老乡鸡优化
        var first_id = sa.store.getFirstId();
        var distinct_id = sa.store.getDistinctId();
        var mp_id = sa.store._state && sa.store._state.identities && sa.store._state.identities.$identity_mp_id;

        if (first_id && first_id === openid && mp_id) {
          sa.store.change('first_id', mp_id);
        }
        if (distinct_id && distinct_id === openid && mp_id) {
          sa.store.change('distinct_id', mp_id);
        }
    ...
    sa.setOpenid = function (openid) {
      //      log('setOpenid is deprecated !!!');

      openid = sa._.validId(openid);
      if (!openid) {
        return false;
      }
      if (!sa.meta.init_status) {
    ...
    sa.initWithOpenid = function (options, callback) {
      ...
      openid.getOpenid(function (openid) {
        if (openid) {
          sa.setOpenid(openid, options.isCoverLogin);
        }
        if (callback && sa._.isFunction(callback)) {
          callback(openid);
        }
        sa.init(options);
      });
  ...
  plugin_name: 'OldOpenid'
};

var OldOpenid = createPlugin(oldOpenid);
...
var onceSend = {
  init(sa) {
    sa.kit.onceSend = function (data) {
      data._flush_time = Date.now();
      var encodeData = sa.kit.onceTrackData(data);
      var url = sa.para.server_url + '?' + encodeData;

      if (sa.para.server_url.indexOf('?') !== -1) {
        url = sa.para.server_url + '&' + encodeData;
      }
  ...
  plugin_name: 'OnceSend'
};

var OnceSend = createPlugin(onceSend);
