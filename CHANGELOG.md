## 1.22.8 (2026-4-17)
1. 修复
    - 修复 $MPPageLeave 事件 event_duration 字段在 page_show_time 异常（0 或 null）时上报为 Unix 秒级时间戳的问题，增加上限阈值检查（> 86400 秒）后归零处理。

## 1.22.7 (2026-3-18)
1. 新增
    - 保证 SDK 中所有的事件的 time 是按照毫秒顺序发送的，如果存在同一时间发送的事件，后触发事件 time 会被自动 +1 毫秒。

## 1.22.6 (2026-2-4)
1. 新增
    - 插件 ABTest 新增功能，$ABtestTrigger 事件先缓存当前 distinct_id ，等请求完成后，继续使用之前缓存的 distinct_id 。
...
## 1.22.4 (2025-10-24)
1. 优化
    - 给出了小程序合规的标准方案。去除了 appLaunch 的缓存机制。 

## 1.22.3 (2025-9-28)
1. 优化
    - 把 SF 即时事件的 get 请求改成 post 请求。
2. 修复
    - v1.22.2 版本中 SF 即时事件插件在 readme 的描述中是配置 instant_events 来筛选事件，实际不生效，现在已修复。
...
## 1.21.10 (2025-7-25)
1. 新增
    - 1.21.8 中 ad-channel 插件支持热启动归因，本次把 ad-channel-h5-linker 也增加进来，且文档中备注了，必须使用 use 替换 usePlugin。 
